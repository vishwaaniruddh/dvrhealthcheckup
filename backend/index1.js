...
app.use(cors({
  origin: 'http://192.168.100.220:5173'
}));
...
app.get('/newallsites_api', (req, res) => {
  ...
  END AS login_status,
  CASE
      WHEN d.login_status = 0 THEN 'Online'
      ELSE 'Offline'
  END AS dvr_status,
DATE_FORMAT(d.last_communication, '%Y-%m-%d %H:%i:%s') AS last_communication,
CASE
...
END AS project,
CASE
WHEN (p.http_port = 'Y') THEN 'working'
ELSE 'not working'
END AS http_port,
CASE
WHEN (p.rtsp_port = 'Y') THEN 'working'
ELSE 'not working'
END AS rtsp_port,  
CASE
WHEN (p.sdk_port = 'Y') THEN 'working'
ELSE 'not working'
END AS sdk_port,  
CASE
WHEN (p.ai_port = 'Y') THEN 'working'
ELSE 'not working'
END AS ai_port
FROM
all_dvr_live d
...
LEFT JOIN 
  port_status_network_report p ON p.site_id = (SELECT SN from port_status WHERE site_sn = s.SN )   
WHERE
  d.live = 'Y'
    `;
...
app.get('/onlinesites_api', (req, res) => {
  ...
  END AS recording_to_status,
         DATE_FORMAT(d.recording_to, '%Y-%m-%d %H:%i:%s') AS recording_to,
          DATE_FORMAT(d.recording_from, '%Y-%m-%d %H:%i:%s') AS recording_from,
          d.dvrname, 
          CASE WHEN (d.hdd = 'Yes' OR d.hdd = 'ok') THEN 'working' ELSE 'not working' 
          END AS hdd_status, 
          CASE WHEN d.login_status = 0 THEN 'working' ELSE 'not working' END AS login_status, 
          DATE_FORMAT(d.last_communication, '%Y-%m-%d %H:%i:%s') AS last_communication,
           CASE
WHEN (s.Bank IS NOT NULL) THEN s.Bank
WHEN (ds.Bank IS NOT NULL) THEN ds.Bank
