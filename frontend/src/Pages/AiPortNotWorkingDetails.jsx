...
const AiPortNotWorkingDetails = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get("http://localhost:2001/ai_not_workingdetails_api", {
                    ...
                    <tbody>
                        {data.map((users, index) => (
                            <tr key={users.ATMID}>
                                ...
                                <td style={{ fontWeight: "bold" }}>{users.rectime}</td>     
...
export default AiPortNotWorkingDetails;
