...
const NewAllSitesDetails = () => {
  ...
  const fetchFolders = async () => {
    try {
      const response = await axios.get("http://192.168.100.220:2001/get-folders"); // Replace with your server URL
      setFolders(response.data); // Update state with folder names
    } catch (err) {
...
const [currentZipPath, setCurrentZipPath] = useState("");
const handleZipFolder = async (folder) => {
  setZipLoading(true);
  setCurrentZipPath("");

  try {
    const response = await fetch(`http://192.168.100.220:2001/start-zip/${folder}`);
    const data = await response.json();

    if (response.ok) {
      setCurrentZipPath(data.zipPath);
    } else {
      console.error(data.message);
      alert("Error zipping folder");
    }
  } catch (err) {
    console.error(err);
    alert("An error occurred while zipping the folder.");
  } finally {
    setZipLoading(false);
  }
};
  ...
  return (
      ...
      <Header />
      {
        loading ? (
          ...
          <div className="container-fluid">
            ...
            <h6>Image Folder List</h6>
    {folders.length > 0 ? (
      <ul>
        {folders.map((folder, index) => (
          <li key={index}>
            <span>{folder}</span>
            <button onClick={() => handleZipFolder(folder)} disabled={zipLoading}>
              {zipLoading ? "Processing..." : "Zip Folder"}
            </button>
          </li>
        ))}
      </ul>
    ...
    {currentZipPath && (
      <div>
        <a href={`http://192.168.100.220:2001${currentZipPath}`} download>
                  ...
                  {data.map((users, index) => (
                    <tr key={users.ATMID}>
                      ...
                      <td>
                        ...
                        <a href={`http://192.168.100.220:2001/public/${new Date().toISOString().split('T')[0]}/${users.ATMID}.jpg`} target="_blank">
                          <img
                            style={{ width: 100, cursor: "pointer" }}
                            src={`http://192.168.100.220:2001/public/${new Date().toISOString().split('T')[0]}/${users.ATMID}.jpg`}
                            alt="Image"
                            onError={(e) => {
                              e.target.onerror = null; // Prevent looping
                              e.target.src = 'https://via.placeholder.com/100?text=No+Image+Available'; // Fallback image or text
                              e.target.alt = 'Image not available'; // Set alt text for better accessibility
                            }}
                        ...
                        {/* Download Link */}
                        <a
                          href={`http://192.168.100.220:2001/download/public/${new Date().toISOString().split('T')[0]}/${users.ATMID}.jpg`}
                          download={`${users.ATMID}.jpg`} // The filename used for downloading
                          style={{
                            display: 'block',
                            marginTop: 5,
                            textAlign: 'center',
                          ...
                          onClick={(e) => {
                            // Optionally handle the error for downloading the fallback image
                            if (e.target.href.includes('placeholder')) {
                              e.preventDefault(); // Prevent download if it's a placeholder
