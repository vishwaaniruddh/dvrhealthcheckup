...
const HddNotExist = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get("http://192.168.100.220:2001/hddnotexist_api", {
    ...
    useEffect(() => {
        ...
        fetch(`http://192.168.100.220:2001/hddnotExist`)
          ...
          .then(data => setHddCount(data.not_exist_hdd_count))
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://192.168.100.220:2001/ExportHddNotExistSites`);
      ...
      const exportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'HDD Not Exist Sites Data');
        XLSX.writeFile(wb, 'HDD_Not_Exist_Sites.xlsx');
    };
    ...
    return (
        <div className="container-fluid">
            <div className='header-sec'>
                <span className='first'>Hdd No Disk Sites</span>
                <span className='second'>Total Hdd No Disk Sites : <span style={{color:"red"}}> {hddCount} </span></span>
